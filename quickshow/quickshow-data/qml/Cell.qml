	...
	*/
	Rectangle {
		...
		function loadData(data)
		{
			...
			textClassName.text = data.classes__name;
			textClassDescription.text = data.courses__length + "/" + data.courses__climb + " m";
	...
	function loadModelData(model) {
		...
		//console.info(model, "cell:", cell)
		var data = model.data(cell.dataIndex);
		if(dataIndex == 0)
			cell.state = "category"
		else
			cell.state = data.type
		//if(dataIndex == 0) console.log("load model data: " + dataIndex + " type: " + data.type + " state: " + cell.state);
		if(cell.state == "category")
			category.loadData(data[cell.state]);
		else if(cell.state == "detail")
			detail.item.loadData(data[cell.state]);
	}

...
#define QF_THROW(exception_type, msg, abort) \
	do { \
		if(abort) \
			qf::core::Log(qf::core::Log::LOG_FATAL, "EXCEPTION") << QString("%1:%2 %3").arg(__FILE__).arg(__LINE__).arg(QF_FUNC_NAME) << msg; \
		else \
...
#define QF_INTERNAL_ERROR(msg, abort)  \
	do { \
		if(abort) { \
			qf::core::Log(qf::core::Log::LOG_FATAL, "QF_INTERNAL_ERROR_EXCEPTION") << QString("%1:%2 %3").arg(__FILE__).arg(__LINE__).arg(QF_FUNC_NAME) <<  msg; \
		} \
		else { \

...
void RunsPlugin::exportResultsHtmlStageWithLaps(const QString &laps_file_name, const qf::core::utils::TreeTable &tt)
{
	...
	{
		QVariantList trr{"tr",
					QVariantList{"th", QVariantMap{{QStringLiteral("class"), "br bbb"}}, "\0xA0"},
					QVariantList{"th", QVariantMap{{QStringLiteral("class"), "br bbb"}}, tr("Registration")},
					QVariantList{"th", QVariantMap{{QStringLiteral("class"), "br bbb"}}, "\0xA0"},
					QVariantList{"th", QVariantMap{{QStringLiteral("class"), "brb bbb"}}, "\0xA0"},
				};
		int i = 1;
		for(QVariant v : course_codes) {
			append_list(trr, QVariantList{"th",
										  QVariantMap{{QStringLiteral("class"), "br bbb"}, {"colspan", "2"}},
										  (i++ < course_codes.size())? QVariant(quickevent::core::CodeDef{v.toMap()}.code()): QVariant("\0xA0")});
		}
		append_list(table, trr);
	}
	...
	for(int j=0; j<tt.rowCount(); j++) {
		...
		append_list(trr2, QVariantList{"td", QVariantMap{{QStringLiteral("class"), "br bb"}}, "\0xA0"});
		...
		append_list(trr2, QVariantList{"td", QVariantMap{{QStringLiteral("class"), "brb bb"}}, "\0xA0"});
		for(int i=0; i<course_codes.size(); i++) {
			QString css_class = QStringLiteral("right");

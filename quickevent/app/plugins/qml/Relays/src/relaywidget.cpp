...
{
	qfLogFuncFrame();
	setPersistentSettingsId(" RelayWidget");
	ui->setupUi(this);
	...
	connect(ui->edFind, &FindRegistrationEdit::registrationSelected, this, & RelayWidget::onRegistrationSelected);
	...
	//connect(ui->tblRuns, &qfw::TableView::customContextMenuRequested, this, & RelayWidget::onRunsTableCustomContextMenuRequest);
...
bool  RelayWidget::loadRunsTable()
{
...
bool  RelayWidget::saveRunsTable()
{
	qfLogFuncFrame();
...
QString  RelayWidget::classNameFromRegistration(const QString &registration)
{
...
void  RelayWidget::onRegistrationSelected(const QVariantMap &values)
{
	qfLogFuncFrame();
	qf::core::model::DataDocument*doc = dataController()->document();
...
void  RelayWidget::loadFromRegistrations(int siid)
{
	qfs::Query q;
...
QVector<int>  RelayWidget::juniorAges()
{
	QVector<int> ret;
	for (int i = 0; i < ui->cbxClass->count(); ++i) {
		QString class_name = ui->cbxClass->itemText(i);
...
QVector<int>  RelayWidget::veteranAges()
{
	QVector<int> ret;
	for (int i = 0; i < ui->cbxClass->count(); ++i) {
		QString class_name = ui->cbxClass->itemText(i);

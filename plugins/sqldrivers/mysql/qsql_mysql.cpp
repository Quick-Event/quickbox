...
#ifdef QF_PATCH
	#ifdef QF_EMBEDDED_MYSQL
		#undef Q_NO_MYSQL_EMBEDDED
	#else
		#define Q_NO_MYSQL_EMBEDDED
	#endif
#endif
...
#if defined QF_PATCH
static QTextCodec* qfCodec(const QString &codec_name_str)
{
	//QTextCodec *ret = QTextCodec::codecForName("UTF-8");
	QByteArray codec_name("UTF-8");
	if(!codec_name_str.isEmpty()) codec_name = codec_name_str.toLatin1();
	QTextCodec *ret = QTextCodec::codecForName(codec_name);
	if(!ret) {
		fprintf(stderr, "************ codec NOT FOUND mysql ver. %i:  '%*s'\n", MYSQL_VERSION_ID, codec_name.size(), codec_name.constData());
		ret = QTextCodec::codecForLocale();
	}
	else {
		//fprintf(stderr, "************ codec() mysql ver. %i: %p '%*s'\n", MYSQL_VERSION_ID, ret, ret->name().size(), ret->name().constData());
	}
	return ret;
}
#endif
...
#if defined QF_PATCH
/// tady jsem pridal parametr len, abych poznal typ bool jako tinyint(1)
#endif
static QVariant::Type qDecodeMYSQLType(int mysqltype, uint flags, unsigned long len)
{
    QVariant::Type type;
    switch (mysqltype) {
    case FIELD_TYPE_TINY :
		if(len == 1 && !(flags & UNSIGNED_FLAG)) {
//fprintf(stderr, "qDecodeMYSQLType: BOOL\n");
			type = QVariant::Bool; break;
		}
...
#if defined QF_PATCH
		/// pokud ma CHAR collation napr utf8_bin, ma binarni flag a QT driver mi ho vraci jako blob
...
static QSqlField qToField(MYSQL_FIELD *field, QTextCodec *tc)
{
	#ifdef QF_PATCH
		QString s, s1;
		s1 = toUnicode(tc, field->db);
		if(!s1.isEmpty()) s += s1 + QLatin1String(".");
		s1 = toUnicode(tc, field->table);
		if(!s1.isEmpty()) s += s1 + QLatin1String(".");
		s += toUnicode(tc, field->name);
//		QSqlField f(s, qDecodeMYSQLType(int(field->type), field->flags, field->length));
		QSqlField f(s,qDecodeMYSQLType(int(field->type), field->flags, field->length));
	#else
                            ...
                            MYSQL_STMT* stmt)
{
    ...
    return QSqlError(QLatin1String("QFMYSQL: ") + err,
...
bool QMYSQLResultPrivate::bindInValues()
{
    ...
    while((fieldInfo = mysql_fetch_field(meta))) {
        QMyField &f = fields[i];
        f.myField = fieldInfo;
        f.type = qDecodeMYSQLType(fieldInfo->type, fieldInfo->flags, fieldInfo->length);
...
QVariant QMYSQLResult::data(int field)
{
    ...
    if (d->preparedQuery) {
        if (f.nullIndicator)
		#if defined QF_PATCH
			/// null je bez typu QVariant(), invalidni datum je QVariant(QDate()).
            return QVariant();
		#else
...
bool QMYSQLResult::reset (const QString& query)
{
    ...
    if (isSelect()) {
        for(int i = 0; i < numFields; i++) {
            ...
            d->fields[i].type = qDecodeMYSQLType(field->type, field->flags, field->length);
        }
...
#if defined QF_PATCH
	fConnectionInfo = QLatin1String("Not connected.");
#endif
}
                         ...
                         const QString& connOpts)
{
    ...
    */
...
#if defined QF_PATCH
    QString qf_codec_name;
    QString qf_set_names;
#endif
    for (int i = 0; i < opts.count(); ++i) {
        ...
        if ((idx = tmp.indexOf(QLatin1Char('='))) != -1) {
            ...
            if (opt == QLatin1String("UNIX_SOCKET"))
                unixSocket = val;
#if defined QF_PATCH
			else if(opt == QLatin1String("QF_CODEC_NAME")) {
					qf_codec_name = val;
				}
			else if(opt == QLatin1String("QF_MYSQL_SET_NAMES")) {
					qf_set_names = val;
				}
#endif
#if MYSQL_VERSION_ID >= 50000
...
#ifdef QF_PATCH
		{
			QString s = QLatin1String("Connected to MySQL server ");
			s += QLatin1String(mysql_get_server_info(d->mysql));
			s += QLatin1String(" ");
			s += QLatin1String(mysql_get_host_info(d->mysql));
			s += QLatin1String(" , ");
			s += QLatin1String(mysql_stat(d->mysql));
			s += QLatin1String("\nCurrent client: ");
			s += QLatin1String(mysql_get_client_info());
			fConnectionInfo = s;
			//setLastError(qMakeError( s, QSqlError::NoError, d));
		}
#endif
	...
	#ifdef QF_PATCH
		if(qf_set_names.isEmpty()) qf_set_names = QLatin1String("utf8");
		//fprintf(stderr, "MYSQL %p\n", d->mysql);
		//fprintf(stderr, "mysql_set_character_set:  '%s'\n", qf_set_names.toLatin1().constData());
		mysql_set_character_set(d->mysql, qf_set_names.toLatin1().constData());
	#else
...
#ifndef QT_NO_TEXTCODEC
	#ifdef QF_PATCH
		d->tc = qfCodec(qf_codec_name);
		//fprintf(stderr, "set text codec:  '%s'\n", d->tc->name().constData());
	#else
		d->tc = codec(d->mysql);
	#endif
#endif
...
//fprintf(stderr, "\t'%s'\n", s.arg(tablename).toStdString().c_str());
	...
	/*
	int ix = tablename.indexOf(QLatin1Char('.'));
	QString tbl = tablename;
	QString db, def_db;
	if(ix > 0) {
		db = tablename.mid(0, ix);
		tbl = tablename.mid(ix+1);
	}
	if(!db.isEmpty()) {
		QSqlQuery q(createResult());
		QString s = QLatin1String("SELECT DATABASE()");
		q.exec(s); q.next();
		def_db = q.value(0).toString();
		if(def_db.isEmpty()) db = QString();
		else {
			s = QLatin1String("USE %1");
			q.exec(s.arg(db));
		}
	}
    MYSQL_RES* r = mysql_list_fields(d->mysql, tbl.toLocal8Bit().constData(), 0);
	if((def_db != db) && !db.isEmpty() && !def_db.isEmpty()) {
		QSqlQuery q(createResult());
		QString s = QLatin1String("USE %1");
		q.exec(s.arg(def_db));
	}
	*/
#else
...
QString QMYSQLDriver::formatValue(const QSqlField &field, bool trimStrings) const
{
    ...
    if (field.isNull()) {
        ...
        case QVariant::Double:
			...
			r = QString::number(field.value().toDouble(), 'g', 16);
            break;
#endif
            // fall through
        default:

...
#ifdef QF_PATCH
QMap<int, QString> pgTableOidMap;
QString QPSQLResult::oid2TableName(int oid) const
{
    QString ret = pgTableOidMap.value(oid);
    if(ret.isNull()) {
		//qfTrash() << QF_FUNC_NAME;
		//qfTrash() << "\tlooking for oid:" << oid;
        /// it is not cashed, try to load it from server
        QString qs = QString::fromLatin1("SELECT c.relname AS tablename, n.nspname AS schemaname"
							" FROM pg_class AS c"
							" LEFT JOIN pg_namespace AS n ON c.relnamespace=n.oid"
							" WHERE c.oid=%1");
        QSqlQuery q(driver()->createResult());
        q.exec(qs.arg(oid)); q.next();
        if(q.isValid()) ret = q.value(1).toString() + QString::fromLatin1(".") + q.value(0).toString();
		//qfTrash() << "\tfound:" << ret;
        pgTableOidMap[oid] = ret;
    }
    return ret;
}
#endif
...
QSqlRecord QPSQLResult::record() const
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (d->privDriver()->isUtf8)
            fs = QString::fromUtf8(PQfname(d->result, i));
        else
            fs = QString::fromLocal8Bit(PQfname(d->result, i));
		QString ts;
		ts = oid2TableName(PQftable(d->result, i));
		if(!ts.isNull()) fs = ts + QChar::fromLatin1('.') + fs;
		f.setName(fs);
#else
        if (d->privDriver()->isUtf8)
